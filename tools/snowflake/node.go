...
const (
	...
	epoch int64 = 1571109245402 // 这个是我在写epoch这个变量时的时间戳(毫秒)
)
...
// 接下来我们开始生成id
// 生成方法一定要挂载在某个node下，这样逻辑会比较清晰 指定某个节点生成id
func (w *Node) Id() int64 {
	// 获取id最关键的一点 加锁 加锁 加锁
	w.mu.Lock()
	...
	ID := int64((now-epoch)<<timeShift | (w.nodeId << workerShift) | (w.number))
	return ID
}
